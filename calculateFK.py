...
class FK():
    ...
    def forward(self, q):
        ...
        def cosine(x):
            # deg1 = math.cos(np.deg2rad(x))
            deg1 = math.cos(x)
            return deg1

        def sine(y):
            # deg2 = math.sin(np.deg2rad(y))
            deg2 = math.sin(y)
            return deg2
        ...
        t1_w = np.array([[cosine(0), -sine(0) * (cosine(alpha[0])), sine(0) * (sine(alpha[0])),
                          a[0] * (cosine(0))], [sine(0), cosine(0) * (cosine(alpha[0])),
                                                   -cosine(0) * (sine(alpha[0])), a[0] * (sine(0))],
                         [0, sine(alpha[0]), cosine(alpha[0]), d[0]], [0, 0, 0, 1]])
        t2_1 = np.array([[cosine(q[0]), -sine(q[0]) * (cosine(alpha[1])), sine(q[0]) * (sine(alpha[1])),
                          a[1] * (cosine(q[0]))], [sine(q[0]), cosine(q[0]) * (cosine(alpha[1])),
                                                   -cosine(q[0]) * (sine(alpha[1])), a[1] * (sine(q[0]))],
                         [0, sine(alpha[1]), cosine(alpha[1]), d[1]], [0, 0, 0, 1]])
        t3_2 = np.array([[cosine(q[1]), -sine(q[1]) * (cosine(alpha[2])), sine(q[1]) * (sine(alpha[2])),
                          a[2] * (cosine(q[1]))], [sine(q[1]), cosine(q[1]) * (cosine(alpha[2])),
                                                   -cosine(q[1]) * (sine(alpha[2])), a[2] * (sine(q[1]))],
                         [0, sine(alpha[2]), cosine(alpha[2]), d[2]], [0, 0, 0, 1]])
        t4_3 = np.array([[cosine(q[2]), -sine(q[2]) * (cosine(alpha[3])), sine(q[2]) * (sine(alpha[3])),
                          a[3] * (cosine(q[2]))], [sine(q[2]), cosine(q[2]) * (cosine(alpha[3])),
                                                   -cosine(q[2]) * (sine(alpha[3])), a[3] * (sine(q[2]))],
                         [0, sine(alpha[3]), cosine(alpha[3]), d[3]], [0, 0, 0, 1]])
        t5_4 = np.array([[cosine(q[3]-pi), -sine(q[3]-pi) * (cosine(alpha[4])), sine(q[3]-pi) * (sine(alpha[4])),
                          a[4] * (cosine(q[3]-pi))], [sine(q[3]-pi), cosine(q[3]-pi) * (cosine(alpha[4])),
                                                    -cosine(q[3]-pi) * (sine(alpha[4])), a[4] * (sine(q[3]-pi))],
                         [0, sine(alpha[4]), cosine(alpha[4]), d[4]], [0, 0, 0, 1]])
        t6_5 = np.array([[cosine(q[4]), -sine(q[4]) * (cosine(alpha[5])), sine(q[4]) * (sine(alpha[5])),
                          a[5] * (cosine(q[4]))], [sine(q[4]), cosine(q[4]) * (cosine(alpha[5])),
                                                   -cosine(q[4]) * (sine(alpha[5])), a[5] * (sine(q[4]))],
                         [0, sine(alpha[5]), cosine(alpha[5]), d[5]], [0, 0, 0, 1]])
        t7_6 = np.array([[cosine(pi-q[5]), -sine(pi-q[5]) * (cosine(alpha[6])), sine(pi-q[5]) * (sine(alpha[6])),
                          a[6] * (cosine(pi-q[5]))], [sine(pi-q[5]), cosine(pi-q[5]) * (cosine(alpha[6])),
                                                   -cosine(pi-q[5]) * (sine(alpha[6])), a[6] * (sine(pi-q[5]))],
                         [0, sine(alpha[6]), cosine(alpha[6]), d[6]], [0, 0, 0, 1]])
        te_7 = np.array([[cosine(q[6]-(pi/4)), -sine(q[6]-(pi/4)) * (cosine(alpha[7])), sine(q[6]-(pi/4)) * (sine(alpha[7])),
                          a[7] * (cosine(q[6]-(pi/4)))], [sine(q[6]-(pi/4)), cosine(q[6]-(pi/4)) * (cosine(alpha[7])),
                                                        -cosine(q[6]-(pi/4)) * (sine(alpha[7])),
                                                        a[7] * (sine(q[6]-(pi/4)))],
                         [0, sine(alpha[7]), cosine(alpha[7]), d[7]], [0, 0, 0, 1]])
